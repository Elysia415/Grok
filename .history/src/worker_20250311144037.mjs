...
const DEFAULT_VISION_MODEL = "grok-2-vision-latest";
const DEFAULT_EMBEDDINGS_MODEL = "grok-embedding-latest";
...
async function transformRequest(req) {
  const config = transformConfig(req);
  
  // 检查是否包含图像，如果有则使用 vision 模型
  const hasImage = req.messages.some(msg => 
    Array.isArray(msg.content) && 
    msg.content.some(item => item.type === "image_url")
  );
  
  if (hasImage && !req.model.includes("vision")) {
    req.model = DEFAULT_VISION_MODEL;
  }
...
function processCompletionsResponse(response, model, id) {
...
async function toOpenAiStream (chunk, controller) {
  ...
  if (this.streamIncludeUsage && data.usage) {
